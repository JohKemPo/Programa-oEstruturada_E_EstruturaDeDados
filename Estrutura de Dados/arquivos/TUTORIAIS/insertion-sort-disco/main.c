...
void imprime_arquivo(FILE *arq) {
    ...
    TFunc *f = le(arq);// associa o primeiro ojeto do tipo TFun a f
    int i = 0;
    while (!feof(arq)) {
        imprime(f);
        f = le(arq);// le o prox struct
    }
}
...
int tamanho_arquivo(FILE *arq) {
    fseek(arq, 0, SEEK_END);// cursor posicionado no fim
    int tam = trunc(ftell(arq) / tamanho_registro()); //posicao final / tamanho total de 1 regisstro
    return tam;
}
