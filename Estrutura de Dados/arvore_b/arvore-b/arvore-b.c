...
int main(int argc, char *argv[]) {
    ...
    int num = 0, from, to;
    while (num != -1) {
        printf("Digite uma chave para ser adicionada na árvore; 0 para imprimir; -9 para buscar e -1 para sair\n");
        scanf("%i", &num);
        if (num == -1) {
            printf("\n");
            imprime(raiz, 0);
            ...
            raiz = insere(raiz, 1, num, NULL);
            printf("Resultado da inserção: \n");
            imprime(raiz, 0);
        }
        printf("\n\n");
    }
