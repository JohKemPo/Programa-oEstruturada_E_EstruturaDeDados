...
typedef struct no{// guarda info
    int valor;
    struct no *proximo;
}No;

typedef struct{// guarda informaçoes para aux manipulaçao
    No *inicio;// ponteiro par astruct no INICIO
    No *fim;
    int tam;
}Lista;

void criar_lista(Lista *lista){// inicia inicio = final = NULL
    lista->inicio = NULL;
    lista->fim = NULL;
    lista->tam = 0;// 0 elementos
}

// procedimento para inserir no início
void inserir_no_inicio(Lista *lista, int num){
    No *novo = malloc(sizeof(No));// cria no novo

    if(novo){
        novo->valor = num; // Lista no setValor
